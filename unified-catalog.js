...
(function () {
  ...
  function normalize(value) {
    return String(value || '').toLowerCase().replace(/[ёë]/g, 'е').replace(/[^a-zа-я0-9]+/g, ' ').trim();
  }

  function material(item) { return (item && item.material_data) || {}; }
  function titleOf(item) { var data = material(item); return data.anime_title || item.title || data.title || 'Без названия'; }
  function posterOf(item) { var data = material(item); return data.anime_poster_url || data.poster_url || ''; }
  function subOf(item) {
    var data = material(item);
    var total = item.last_episode || data.episodes_aired || data.episodes_total || '';
    return [item.year || data.year, data.anime_kind || data.kind, total ? total + ' эп.' : ''].filter(Boolean).join(' · ');
  }
  ...
  function cardHtml(item) {
    ...
    return '<a class="card uc-kodik" data-uc-source="kodik" href="#/kodik/' + encodeURIComponent(item.id) + '">' +
      '<div class="card-poster">' + (poster ? '<img src="' + escapeHtml(poster) + '" alt="" loading="lazy" decoding="async">' : '') +
      '<span class="card-badge">Kodik</span></div><div class="card-body"><div class="card-title">' +
      escapeHtml(titleOf(item)) + '</div><div class="card-sub">' + escapeHtml(subOf(item)) + '</div></div></a>';
  }

  function isCatalog() { return /^#\/catalog(?:\?|$)/.test(location.hash || ''); }
  function query() {
  ...
  function requestPlan() {
    ...
    var request = { limit: 50, types: 'anime,anime-serial', with_material_data: true };
    if (search) request.title = search;
    else { request.sort = 'updated_at'; request.order = 'desc'; }
  ...
  function waitForKodik(tries) {
    ...
    return new Promise(function (resolve) { setTimeout(resolve, 120); }).then(function () { return waitForKodik((tries || 0) + 1); });
  }

  function loadKodik() {
    if (!isCatalog() || loading) return;
    var key = location.hash || '#/catalog';
    if (key === cacheKey && kodikItems.length) { scheduleRender(); return; }
    loading = true;
    cacheKey = key;
    kodikItems = [];
    updateToolbar('Загружаем Kodik…');
    var plan = requestPlan();
    waitForKodik().then(function (api) { return api.request(plan.path, plan.params); }).then(function (data) {
      if (cacheKey !== key) return;
      kodikItems = unique(data && data.results).slice(0, 30);
    }).catch(function () {
      if (cacheKey === key) kodikItems = [];
    }).then(function () { loading = false; scheduleRender(); });
  }
  ...
  function toolbarHost() { return document.querySelector('#view-list .catalog-main'); }
  function ensureToolbar() {
    ...
    if (!toolbar) {
      ...
      toolbar.innerHTML = '<span class="uc-label">Источники каталога</span><div class="uc-sources" role="group" aria-label="Источник каталога">' +
  ...
  function appendKodik(grid) {
    ...
    var html = kodikItems.filter(function (item) { return !aniNames[normalize(titleOf(item))]; }).map(cardHtml).join('');
  ...
  function route() {
    var toolbar = document.getElementById('ucToolbar');
    if (!isCatalog()) { if (toolbar) toolbar.remove(); return; }
    injectCss();
    cacheKey = '';
    kodikItems = [];
    ensureToolbar();
