...
const SHELL = ['./','index.html','style.css','mobile-app.css','config.js','db.js','gamify.js','app.js','auth.js','social.js','extras.js','kodik.js','polish.js','kodik-net.js','mobile-nav.js','unified-catalog.js','mobilefix.js','offline.js','schedule.js','kodik-browse.js','catalog.js','player-plus.js','manifest.webmanifest']
...
async function shellFirst(req){try{const response=await fetch(req,{cache:'no-store'});if(response&&response.ok&&response.type==='basic')caches.open(CACHE).then((cache)=>cache.put(req,response.clone())).catch(()=>{});return response}catch{const hit=await fromShell(req);if(hit)return hit;if(req.mode==='navigate')return(await caches.match('index.html',{ignoreSearch:true}))||Response.error();return Response.error()}}
